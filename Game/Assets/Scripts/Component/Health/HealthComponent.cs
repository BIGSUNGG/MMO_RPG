...
public class HealthComponent : ObjectComponent
{
    ...
    // victim : 대미지를 받을 오브젝트
    // return : 최종적으로 준 대미지
    public int OnServer_GiveDamage(ObjectController victim, float damage)
    {
    ...
    // return : 최종적으로 받은 대미지
    public int OnServer_TakeDamage(ObjectController attacker, float damage) 
    {
        if (Util.CheckFuncCalledOnServer() == false) // 서버에서 호출되지않은 경우
            return 0;
        ...
        try
        {
            switch (functionId)
            {
                case RpcComponentFunctionId.MulticastTakeDamage:
                    ...
                    break;
                case RpcComponentFunctionId.MulticastGiveDamage:
                    break;
